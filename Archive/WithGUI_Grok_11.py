...
def calculate_trend_consistency(t, q, mask, popt):
    ...
    # FIX: Add a check for the minimum number of points required for linear regression.
    # We use 3 as a safe minimum, but anything >= 2 will prevent the NaN error.
    if len(q_actual) < 3:
        return 0  # Return a neutral score if there's not enough data
    ...
    # Handle potential NaN from linregress if sample size is still too small after slicing
...
def compute_metrics(df_all, well_name, manual_start_idx=0, filter_params=None, decline_params=None, period_params=None, ignore_gaps=False):
...
def run_arps_for_well_auto(df_all, well_name, outlier_threshold=2, 
    ...
    if len(df_well) > 1:
        ...
        # Detect gaps only if not manual
        if start_method != "Manual" and large_gaps.any():
            # Convert to numeric index
            last_gap_idx = np.where(large_gaps.values)[0][-1]  # robust and index-safe
            points_after_gap = len(df_well) - (last_gap_idx + 1)
...
class DeclineCurveApp(tk.Tk):
    ...
    def run_analysis(self):
        ...
        # Generate pbounds for optimization
        pbounds = {}
        param_to_opt_var = {
            ...
            if pbounds:
                def objective_function(**params):
                    ...
                    metrics = compute_metrics(
                        ...
                        ignore_gaps=(start_method == "Manual")
                    )
