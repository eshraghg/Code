...
def filter_outliers_iterative(t, q, threshold=3, max_iterations=MAX_OUTLIER_ITERATIONS):
    ...
    for iteration in range(max_iterations):
        ...
        # FIXED: Protect trailing data points from being marked as outliers
        # Find the last non-outlier index
        ...
        # If we found a non-outlier, restore all points after it
        if last_non_outlier_idx >= 0 and last_non_outlier_idx < n - 1:
            # Check if there's a contiguous block of outliers at the end
            has_trailing_outliers = not new_mask[n - 1]
            if has_trailing_outliers:
                # Restore all trailing outliers (from last non-outlier to end)
...
def create_arps_plot(df_well, t, q, mask, popt, well_id, df_full, q_original, q_smoothed, outlier_threshold, forecast_avg_points=1, start_method=""):
    ...
    # FIXED: Consistent forecast time array
...
class DeclineCurveApp(tk.Tk):
    def __init__(self):
        ...
        # FIXED: Store custom parameter ranges
        ...
        # FIXED: Add button to configure optimization ranges
    ...
    def create_general_tab(self):
        ...
        # FIXED: Corrected variable names to match optimization parameters
    ...
    def open_range_config(self):
        """Open dialog to configure optimization parameter ranges"""
    ...
    def run_analysis(self):
        ...
        if optimize:
            ...
            # FIXED: Correct parameter-to-checkbox mapping
            param_to_opt_var = {
            ...
            if pbounds:
                # FIXED: Simplified objective function
                def objective_function(**params):
                    for param in int_keys:
                        if param in params:
                            params[param] = int(round(params[param]))
                    
                    # Use predetermined start index
