...
def calculate_fit_quality(t, q, mask, popt):
    ...
    # === MODIFICATION: Penalize very low Di ===
    Di_penalty = 1.0
    if Di < 0.001:
        Di_penalty = 0.01
    elif Di < 0.01:
...
class DeclineCurveApp(tk.Tk):
    def __init__(self):
        ...
        # === NEW BUTTONS ===
        self.select_start_btn = ttk.Button(self.left_frame, text="Select Start Date on Chart", command=self.enable_chart_click_selection)
        self.select_start_btn.pack(pady=5)
        self.select_qi_btn = ttk.Button(self.left_frame, text="Select Start Date & Qi on Chart", command=self.enable_qi_click_selection)
        self.select_qi_btn.pack(pady=5)
    ...
    def run_analysis(self):
        ...
        # Reset fixed_qi unless in Qi mode
        ...
        if optimize:
            ...
            if pbounds:
                def objective_function(**params):
                    ...
                    # === Di Penalty in Objective Function ===
