...
def create_arps_plot(df_well, t, q, mask, popt, well_id, df_full, q_original, q_smoothed, 
                     ...
                     forecast_offset=0):  # Changed show_smoothed default to False, added show_channel, added forecast settings
    if popt is None:
    ...
    # shift forecast dates to the right by forecast_offset months (horizontal shift only)
    forecast_dates = pd.date_range(start=last_date + pd.DateOffset(months=1 + int(max(0, forecast_offset))),
                                   periods=forecast_months, freq='MS')
    
    last_t = t[-1]
    # Keep model time continuous from last_t (do not change physics), only dates are shifted visually
    ...
    # Determine the start index of decline phase in the full dataset
    decline_start_idx = len(q_original) - len(q)
    
    if show_pre_decline and decline_start_idx > 0:
        # Only show original data points BEFORE the decline phase
    ...
    # Only show channel if the checkbox is checked
    if show_channel:
...
def run_arps_for_well_auto(df_all, well_name, outlier_threshold=2, 
                           ...
                           forecast_duration=60, forecast_offset=0):  # Added show_channel, duration, and offset
    df_well = df_all[df_all['Well_Name'] == well_name].copy()
    if df_well.empty:
...
class DeclineCurveApp(tk.Tk):
    def __init__(self):
        ...
        # Store current analysis parameters for plot updates
    ...
    def create_plot_options_tab(self):
        ...
        # Changed to unchecked by default
        ...
        # New checkbox for showing channel (unchecked by default)
    ...
    def update_plot_options(self):
        ...
        try:
            ...
            # Also get current offset in case user changed it
            current_offset = max(0, min(120, int(self.forecast_offset_var.get())))
            
            # Recreate the plot with new options using stored parameters
            ...
            # Don't show error message to user for plot option updates
    ...
    def run_analysis(self):
        ...
        show_forecast = self.show_forecast_var.get()
        show_smoothed = self.show_smoothed_var.get()
        show_channel = self.show_channel_var.get()  # New variable
