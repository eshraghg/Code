...
def calculate_fit_quality(t, q, mask, popt):
    ...
    # === MODIFICATION 1: Penalize very low Di ===
    Di_penalty = 1.0
    if Di < 0.001:  # e.g., less than 0.1% monthly decline
        Di_penalty = 0.01
    elif Di < 0.01:
        Di_penalty = 0.3
    elif Di < 0.05:
...
class DeclineCurveApp(tk.Tk):
    def __init__(self):
        ...
        # === MODIFICATION 2: Add manual click selection button ===
        self.select_on_chart_btn = ttk.Button(self.left_frame, text="Select Start Date on Chart", command=self.enable_chart_click_selection)
        self.select_on_chart_btn.pack(pady=5)
    ...
    def on_chart_click(self, event):
        if not self.click_selection_enabled or event.inaxes is None:
            return
        clicked_date = event.xdata  # matplotlib date number
        ...
        self.start_method_var.set("Manual")
        ...
        if optimize:
            ...
            if pbounds:
                def objective_function(**params):
                    ...
                    # === MODIFICATION 1: Apply Di penalty in objective function ===
