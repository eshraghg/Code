...
def compute_metrics(df_all, well_name, manual_start_idx=0, filter_params={}):
    ...
    mask, popt = filter_outliers_iterative(
        ...
        max_iterations=int(filter_params.get('max_iterations', 5))
    )
...
class DeclineCurveApp(tk.Tk):
    def __init__(self):
        ...
        self.outlier_threshold_opt_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(general_tab, text="", variable=self.outlier_threshold_opt_var).grid(row=0, column=0, sticky=tk.W)
        ttk.Label(general_tab, text="Outlier Threshold:").grid(row=0, column=1, sticky=tk.W)
        ttk.Entry(general_tab, textvariable=self.outlier_threshold_var).grid(row=0, column=2, pady=2)
        ...
        ttk.Entry(general_tab, textvariable=self.forecast_avg_points_var).grid(row=1, column=2, pady=2)
        
        self.filter_max_iter_var = tk.IntVar(value=5)
        self.filter_max_iter_opt_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(general_tab, text="", variable=self.filter_max_iter_opt_var).grid(row=2, column=0, sticky=tk.W)
        ttk.Label(general_tab, text="Max Outlier Iterations:").grid(row=2, column=1, sticky=tk.W)
        ttk.Entry(general_tab, textvariable=self.filter_max_iter_var).grid(row=2, column=2, pady=2)
        ...
        ttk.Entry(decline_tab, textvariable=self.smooth_window_var).grid(row=0, column=2, pady=2)
        ...
        ttk.Entry(decline_tab, textvariable=self.polyorder_var).grid(row=1, column=2, pady=2)
        ...
        ttk.Entry(decline_tab, textvariable=self.peak_prominence_var).grid(row=2, column=2, pady=2)
        ...
        ttk.Entry(decline_tab, textvariable=self.post_ma_var).grid(row=3, column=2, pady=2)
        ...
        ttk.Entry(decline_tab, textvariable=self.decline_frac_var).grid(row=4, column=2, pady=2)
        ...
        ttk.Entry(decline_tab, textvariable=self.post_slope_win_var).grid(row=5, column=2, pady=2)
        ...
        ttk.Entry(decline_tab, textvariable=self.persist_months_var).grid(row=6, column=2, pady=2)
        
        self.min_decline_months_var = tk.IntVar(value=18)
        self.min_decline_months_opt_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(decline_tab, text="", variable=self.min_decline_months_opt_var).grid(row=7, column=0, sticky=tk.W)
        ttk.Label(decline_tab, text="Min Decline Months:").grid(row=7, column=1, sticky=tk.W)
        ttk.Entry(decline_tab, textvariable=self.min_decline_months_var).grid(row=7, column=2, pady=2)
        
        self.drop_frac_var = tk.DoubleVar(value=0.2)
        self.drop_frac_opt_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(decline_tab, text="", variable=self.drop_frac_opt_var).grid(row=8, column=0, sticky=tk.W)
        ttk.Label(decline_tab, text="Drop Frac:").grid(row=8, column=1, sticky=tk.W)
        ttk.Entry(decline_tab, textvariable=self.drop_frac_var).grid(row=8, column=2, pady=2)
        ...
        self.min_prod_months_opt_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(period_tab, text="", variable=self.min_prod_months_opt_var).grid(row=0, column=0, sticky=tk.W)
        ttk.Label(period_tab, text="Min Production Months:").grid(row=0, column=1, sticky=tk.W)
        ttk.Entry(period_tab, textvariable=self.min_prod_months_var).grid(row=0, column=2, pady=2)
        
        self.surge_mult_var = tk.DoubleVar(value=2.0)
        self.surge_mult_opt_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(period_tab, text="", variable=self.surge_mult_opt_var).grid(row=1, column=0, sticky=tk.W)
        ttk.Label(period_tab, text="Surge Multiplier:").grid(row=1, column=1, sticky=tk.W)
        ttk.Entry(period_tab, textvariable=self.surge_mult_var).grid(row=1, column=2, pady=2)
        
        self.gap_thresh_var = tk.DoubleVar(value=0.3)
        self.gap_thresh_opt_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(period_tab, text="", variable=self.gap_thresh_opt_var).grid(row=2, column=0, sticky=tk.W)
        ttk.Label(period_tab, text="Gap Threshold:").grid(row=2, column=1, sticky=tk.W)
        ttk.Entry(period_tab, textvariable=self.gap_thresh_var).grid(row=2, column=2, pady=2)
        
        self.lookback_win_var = tk.IntVar(value=6)
        self.lookback_win_opt_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(period_tab, text="", variable=self.lookback_win_opt_var).grid(row=3, column=0, sticky=tk.W)
        ttk.Label(period_tab, text="Lookback Window:").grid(row=3, column=1, sticky=tk.W)
        ttk.Entry(period_tab, textvariable=self.lookback_win_var).grid(row=3, column=2, pady=2)
    ...
    def run_analysis(self):
        ...
        # Gather parameters and optimization flags
        filter_params = {
        ...
        if optimize:
            ...
            if start_method == "Auto Select":
                ...
                for param in decline_ranges:
                    if getattr(self, f"{param}_opt_var").get():
                        pbounds[param] = decline_ranges[param]
                    else:
                        fixed_params[param] = decline_params[param]
                for param in period_ranges:
                    if getattr(self, f"{param}_opt_var").get():
                        pbounds[param] = period_ranges[param]
                    else:
                        fixed_params[param] = period_params[param]
                for param in filter_ranges:
                    if getattr(self, f"{param}_opt_var").get():
                        pbounds[param] = filter_ranges[param]
                    else:
                        fixed_params[param] = filter_params[param]
                n_iter = 20
            ...
            if pbounds:
                def objective_function(**params):
                    ...
                    if start_method == "Auto Select":
                        sampled_decline = {k: params.get(k, fixed_params[k]) for k in decline_ranges}
                        sampled_period = {k: params.get(k, fixed_params[k]) for k in period_ranges}
                        sampled_filter = {k: params.get(k, fixed_params[k]) for k in filter_ranges}
                        ...
                        sampled_filter = {k: params.get(k, fixed_params[k]) for k in filter_ranges}
                        metrics = compute_metrics(self.df_all, well_name, manual_start_idx=manual_start_idx, filter_params=sampled_filter)
                ...
                # Update GUI fields with optimized values
                for param, value in best_params.items():
                    getattr(self, f"{param}_var").set(round(value, 2) if param not in int_keys else int(value))
                
                # Assign optimized parameters
                if start_method == "Auto Select":
                    best_decline = {k: best_params.get(k, fixed_params[k]) for k in decline_ranges}
                    best_period = {k: best_params.get(k, fixed_params[k]) for k in period_ranges}
                    best_filter = {k: best_params.get(k, fixed_params[k]) for k in filter_ranges}
