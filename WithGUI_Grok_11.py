...
def run_arps_for_well_auto(df_all, well_name, outlier_threshold=2, 
    ...
    # Check for 10+ year gaps in the data
    if len(df_well) > 1:
        date_diffs = df_well['Prod_Date'].diff()
        large_gaps = date_diffs > pd.Timedelta(days=365*10)
        
        if large_gaps.any():
            # Find the last large gap
            last_gap_idx = large_gaps[::-1].idxmax()  # Get index of last True value
            points_after_gap = len(df_well) - last_gap_idx
            
            if points_after_gap >= 10:
                gap_date_before = df_well.iloc[last_gap_idx - 1]['Prod_Date']
                gap_date_after = df_well.iloc[last_gap_idx]['Prod_Date']
                ...
                print(f"Excluding {last_gap_idx} points before the gap")
                print(f"{'!'*60}\n")
                
                df_well = df_well.iloc[last_gap_idx:].reset_index(drop=True)
                start_idx += last_gap_idx  # Update start_idx for plotting purposes
