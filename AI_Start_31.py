...
def run_arps_for_well_auto(df_all, well_name, outlier_threshold=2, 
    ...
    df_well = df_all[df_all['Well_Name'] == well_name].copy()
    if df_well.empty:
        return None, "No data found for well", None
    df_well = df_well.sort_values('Prod_Date')
    ...
    model_data = None
    if popt is not None and fig is not None:
        ...
        # Apply forecast adjustments (same logic as in create_arps_plot)
        if forecast_avg_points > 1:
            n_points = min(forecast_avg_points, np.sum(mask))
            initial_rate = np.mean(q[mask][-n_points:])
            fitted_rate_at_last_t = arps_hyperbolic(last_t, qi_original, Di, b)
        ...
        # Ensure popt is a numpy array for .tolist() method
        if isinstance(popt, list):
            popt = np.array(popt)
        
        model_data = {
            'popt': popt.tolist() if hasattr(popt, 'tolist') else list(popt),  # [qi, Di, b]
            'mask': mask.tolist(),
            't': t.tolist(),
            'q': q.tolist(),
            'df_well_prod_dates': df_well['Prod_Date'].dt.strftime('%Y-%m-%d').tolist(),
            'df_full_prod_dates': df_full['Prod_Date'].dt.strftime('%Y-%m-%d').tolist(),
            'q_original_full': q_original_full.tolist(),
            'q_smoothed_full': q_smoothed_full.tolist(),
            'forecast_dates': [d.strftime('%Y-%m-%d') for d in forecast_dates],
            'forecast_q': forecast_q.tolist(),
            'start_idx': int(start_idx),
            'decline_start_date': df_well['Prod_Date'].min().strftime('%Y-%m-%d') if len(df_well) > 0 else None
        }
...
class DeclineCurveApp(QMainWindow):
    ...
    def create_analysis_data(self, well_name):
        ...
        if not self.current_analysis_params:
            return None
        
        # Use consistent key for well_name, especially for aggregated wells
        if hasattr(self, 'applied_wells') and self.applied_wells and len(self.applied_wells) > 1:
            ...
            if analysis_key:
                well_name_for_data = analysis_key
            else:
                well_name_for_data = well_name
        else:
            well_name_for_data = well_name
        
        # Check if current_well_name matches (now uses consistent key)
        if self.current_well_name and self.current_well_name != well_name_for_data and self.current_well_name != well_name:
            return None
        
        # Get current GUI state
        # Get hierarchy for the well (for single wells) or from applied_wells
        if hasattr(self, 'applied_wells') and self.applied_wells and len(self.applied_wells) > 1:
            # For aggregated wells, get hierarchy from first well
            primary_well = self.applied_wells[0] if self.applied_wells else well_name
            district, field, formation = self.get_well_hierarchy(primary_well)
        else:
            district, field, formation = self.get_well_hierarchy(well_name)
        
        data = {
            'timestamp': datetime.now().isoformat(),
            'well_name': well_name_for_data,
    ...
    def save_current_well(self):
        ...
        try:
            ...
            # Also store in session and mark as saved using consistent key
            ...
            self.session_analyses[analysis_key] = data
            self.saved_analyses[analysis_key] = True
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if selected_idx >= 0:
                ...
                if data:
                    ...
                    # Use consistent key for aggregated wells
                    applied_wells_from_data = data.get('applied_wells', [data.get('well_name', well_name)])
                    analysis_key = self._get_analysis_key(applied_wells_from_data)
                    if not analysis_key:
                        analysis_key = data.get('well_name', well_name)
                    self.session_analyses[analysis_key] = data
                    self.saved_analyses[analysis_key] = True  # Loaded from file, so it's saved
                    
                    # Check if we have saved model_data - if so, restore plot directly
                    if 'model_data' in data and data['model_data'] is not None:
                        if self.restore_plot_from_model_data(data):
    ...
    def load_all_latest(self):
            ...
            for field_dir in district_dir.iterdir():
                ...
                for formation_dir in field_dir.iterdir():
                    ...
                    for well_dir in formation_dir.iterdir():
                        ...
                        if data:
                            # Get the actual well name from the data and use consistent key
                            applied_wells_from_data = data.get('applied_wells', [data.get('well_name', well_name_sanitized)])
                            analysis_key = self._get_analysis_key(applied_wells_from_data)
                            if not analysis_key:
                                analysis_key = data.get('well_name', well_name_sanitized)
                            self.session_analyses[analysis_key] = data
                            self.saved_analyses[analysis_key] = True  # Loaded from file, so it's saved
                            loaded_count += 1
                            
                            # Check if this is the current well (by checking applied_wells match)
                            current_analysis_key = self._get_analysis_key(self.applied_wells) if (hasattr(self, 'applied_wells') and self.applied_wells) else current_well_name
                            if analysis_key == current_analysis_key or analysis_key == current_well_name:
        ...
        if current_well_data:
            ...
            # Check if we have saved model_data - if so, restore plot directly
            if 'model_data' in current_well_data and current_well_data['model_data'] is not None:
                if self.restore_plot_from_model_data(current_well_data):
                    results_text = current_well_data.get('results_text', '')
                    if results_text:
                        self.results_text.clear()
                        self.results_text.setText(results_text)
                    self.current_well_name = current_well_name
                    analysis_params = current_well_data.get('analysis_params', {})
                    if analysis_params:
                        self.current_analysis_params = analysis_params.copy()
                    self.update_vertical_line_from_inputs()
                    QApplication.processEvents()
                    ...
                    # Fallback to running analysis if restore failed
                    QApplication.processEvents()
                    self.run_analysis()
                    self._restore_references_after_analysis()
            else:
    ...
    def restore_plot_from_model_data(self, data):
        """Restore plot from saved model_data without re-running analysis"""
        if not data or 'model_data' not in data:
            return False
    ...
    def load_analysis_from_session(self, well_name):
        ...
        try:
            ...
            if self.apply_analysis_data(data):
                # Check if we have saved model_data - if so, restore plot directly
                if 'model_data' in data and data['model_data'] is not None:
                    # Restore plot from saved model_data
                    restore_success = self.restore_plot_from_model_data(data)
                    if restore_success:
                        # Set results text
                        results_text = data.get('results_text', '')
                        if results_text:
                        ...
                        # Set current_well_name and current_analysis_params from saved data
                        # Use consistent key for aggregated wells
                        if len(applied_wells) > 1:
                        ...
                        if analysis_params:
                            ...
                            if 'outlier_threshold' not in self.current_analysis_params:
                                self.current_analysis_params['outlier_threshold'] = float(data.get('threshold', 2.0))
                            if 'forecast_duration' not in self.current_analysis_params:
                                self.current_analysis_params['forecast_duration'] = int(data.get('forecast_duration', 60))
                        
                        # Redraw vertical line if in manual mode
                        self.update_vertical_line_from_inputs()
                        
                        # Ensure reference list is updated before restoring references
                        # This is especially important when loading immediately after file load
                        self.update_reference_list()
                        QApplication.processEvents()
    ...
    def update_reference_list(self):
        ...
        for well_name, data in sorted(self.session_analyses.items()):
            ...
            # Use _build_selection_title for aggregated wells to match chart titles
            applied_wells = data.get('applied_wells', [well_name])
            if len(applied_wells) > 1:
                hierarchy_name = self._build_selection_title(applied_wells)
            else:
    ...
    def _final_restore_references(self, active_references):
        ...
        # Ensure reference list is up to date (might have been cleared or not ready)
        # Only update if needed to avoid clearing selections unnecessarily
        current_count = self.reference_list.count()
        expected_count = len([w for w in self.session_analyses.keys() if w != self.current_well_name])
        if current_count != expected_count:
            # Reference list needs updating
            self.update_reference_list()
            QApplication.processEvents()
        
        # Restore the references directly
    ...
    def _restore_references_after_analysis(self):
        ...
        if hasattr(self, '_pending_references_restore'):
            ...
            # Normalize reference keys to match what's in session_analyses and reference list
            normalized_references = []
            for ref_key in active_references:
                if isinstance(ref_key, dict):
                    ref_key = ref_key.get('well_name', ref_key)
                
                # Try direct lookup first
                if ref_key in self.session_analyses:
                    normalized_references.append(ref_key)
                else:
                    # If not found, try to find by matching applied_wells or well_name
                    found_key = None
                    for key, key_data in self.session_analyses.items():
                        saved_wells = key_data.get('applied_wells', [key])
                        # For single wells, check if well_name matches
                        if len(saved_wells) == 1 and saved_wells[0] == ref_key:
                            found_key = key
                            break
                        # For aggregated wells, check if well_name matches
                        elif key_data.get('well_name') == ref_key:
                            found_key = key
                            break
                    
                    if found_key:
                        normalized_references.append(found_key)
                    else:
                        # If still not found, try original key (might work if list widget has it)
                        normalized_references.append(ref_key)
            
            # Ensure reference list is up to date first
            # This is critical when loading immediately after file load
            self.update_reference_list()
            
            # Process events multiple times to ensure the list is fully built and UI is ready
            QApplication.processEvents()
            QApplication.processEvents()  # Extra processing for immediate loads
            
            # Restore references synchronously - the list should be ready now
            # Use a small delay even when not loading to ensure UI is fully ready
            # This fixes the issue when loading immediately after file load
            QTimer.singleShot(50, lambda: self._final_restore_references(normalized_references))
    ...
    def update_plot_options(self):
        ...
        try:
            ...
            # Try to restore from saved model_data first (faster than re-running analysis)
            ...
            if analysis_key and analysis_key in self.session_analyses:
                ...
                if 'model_data' in data and data['model_data'] is not None:
                    # Update the saved data with new plot options
                    data['show_outliers'] = show_outliers
                    data['show_pre_decline'] = show_pre_decline
                    data['show_forecast'] = show_forecast
                    data['show_inliers'] = show_inliers
                    data['show_channel'] = show_channel
                    data['active_references'] = [ref['well_name'] for ref in reference_models]
                    
                    # Restore plot with updated options
                    if self.restore_plot_from_model_data(data):
            ...
            fig, results, model_data = run_arps_for_well_auto(
            ...
            if fig:
                ...
                # Ensure canvas is refreshed immediately
                if self.canvas:
                    self.canvas.draw()
                    QApplication.processEvents()  # Force immediate UI update
                ...
                # Update model_data in session if available using consistent key
                if model_data is not None and hasattr(self, 'applied_wells') and self.applied_wells:
                    analysis_key = self._get_analysis_key(self.applied_wells)
                    if analysis_key and analysis_key in self.session_analyses:
                        self.session_analyses[analysis_key]['model_data'] = self.convert_to_json_serializable(model_data)
                        self.saved_analyses[analysis_key] = False  # Mark as unsaved since we updated it
    ...
    def apply_selection(self):
        ...
        # Check if there's a matching analysis in the session using consistent key
        analysis_key = self._get_analysis_key(selected_wells)
        matching_analysis = None
        if analysis_key and analysis_key in self.session_analyses:
            matching_analysis = self.session_analyses[analysis_key]
        else:
            # Fallback: check by applied_wells set for backward compatibility
            for key, data in self.session_analyses.items():
                saved_wells = data.get('applied_wells', [])
                if set(saved_wells) == set(selected_wells):
                    matching_analysis = data
                    # Update to use new key format
                    self.session_analyses[analysis_key] = data
                    self.saved_analyses[analysis_key] = self.saved_analyses.get(key, False)
                    if key != analysis_key:
                        del self.session_analyses[key]
                        if key in self.saved_analyses:
                            del self.saved_analyses[key]
                    break
        ...
        if matching_analysis:
            if self.apply_analysis_data(matching_analysis):
                # Check if we have saved model_data - if so, restore plot directly
                if 'model_data' in matching_analysis and matching_analysis['model_data'] is not None:
                    restore_success = self.restore_plot_from_model_data(matching_analysis)
                    if restore_success:
                        # Set results text
                        results_text = matching_analysis.get('results_text', '')
                        if results_text:
                            self.results_text.clear()
                            self.results_text.setText(results_text)
                        ...
                        # Set current_well_name and current_analysis_params from saved data
                        if len(selected_wells) > 1:
                            self.current_well_name = analysis_key
                        else:
                            self.current_well_name = selected_wells[0] if selected_wells else None
                        
                        analysis_params = matching_analysis.get('analysis_params', {})
                        if analysis_params:
                            self.current_analysis_params = analysis_params.copy()
                            # Ensure all required keys exist
                            if 'outlier_threshold' not in self.current_analysis_params:
                                self.current_analysis_params['outlier_threshold'] = float(matching_analysis.get('threshold', 2.0))
                            if 'forecast_duration' not in self.current_analysis_params:
                                self.current_analysis_params['forecast_duration'] = int(matching_analysis.get('forecast_duration', 60))
                ...
                # If no model_data or restore failed, fall back to running analysis
    ...
    def run_analysis_internal(self, well_name):
        ...
        if fig:
            ...
            # Use consistent key for current_well_name (especially for aggregated wells)
            if hasattr(self, 'applied_wells') and self.applied_wells and len(self.applied_wells) > 1:
                self.current_well_name = self._get_analysis_key(self.applied_wells)
            else:
                self.current_well_name = well_name
            ...
            # Store analysis in session using consistent key
            analysis_key = self._get_analysis_key(self.applied_wells) if hasattr(self, 'applied_wells') and self.applied_wells else well_name
            analysis_data = self.create_analysis_data(well_name)
            if analysis_data:
                # Store model_data if available (it should be from run_arps_for_well_auto call above)
                # Check if model_data exists in the current scope - it should be available from run_arps_for_well_auto
                try:
                    if model_data is not None:
                        analysis_data['model_data'] = self.convert_to_json_serializable(model_data)
                except NameError:
                    # model_data not available - this can happen if analysis failed or in some edge cases
                    pass
                
                # Extract qi, Di, b from model_data or results_text
                qi_val = None
                di_val = None
                b_val = None
                
                if 'model_data' in analysis_data and 'popt' in analysis_data['model_data']:
                    popt = analysis_data['model_data']['popt']
                    analysis_data['qi'] = popt[0]
                    analysis_data['Di'] = popt[1]
                    analysis_data['b'] = popt[2]
                else:
                    # Fallback to parsing results_text
                    results_text = self.results_text.toPlainText()
                ...
                # Store the decline start date from model_data if available
                if 'model_data' in analysis_data and 'decline_start_date' in analysis_data['model_data']:
                    analysis_data['decline_start_date'] = analysis_data['model_data']['decline_start_date']
                elif self.current_analysis_params and well_name:
                    start_idx = self.current_analysis_params.get('manual_start_idx', 0)
                    if len(self.applied_wells) > 1 and self.df_aggregated_cache is not None:
                ...
                self.session_analyses[analysis_key] = analysis_data
                self.saved_analyses[analysis_key] = False  # Mark as unsaved
    ...
    def _get_analysis_key(self, well_names):
        """Get a consistent key for storing/retrieving analyses based on well_names.
        For single wells, returns the well name. For aggregated wells, returns a consistent format."""
        if not well_names:
            return None
        if len(well_names) == 1:
            return well_names[0]
        
        # For aggregated wells, use the same logic as _build_selection_title but without "Well: " prefix
        ...
        # Fallback - use sorted list of well names as key for unique identification
        sorted_wells = sorted(well_names)
        return f"Aggregated ({len(well_names)} wells): {', '.join(sorted_wells[:3])}{'...' if len(sorted_wells) > 3 else ''}"
