...
class DeclineCurveApp(QMainWindow):
    ...
    def save_all_wells(self):
        """Save analyses for all wells currently in session"""
        ...
        for well_name, data in self.session_analyses.items():
            ...
            try:
                with open(filepath, 'w') as f:
                    json.dump(data, f, indent=2)
                saved_count += 1
                self.saved_analyses[well_name] = True  # Mark as saved
            except Exception as e:
                failed_wells.append(well_name)
                print(f"Failed to save {well_name}: {e}")
        
        if failed_wells:
            QMessageBox.warning(self, "Partial Success", 
                               f"Saved {saved_count} well(s).\n\n"
                               f"Failed to save: {', '.join(failed_wells)}")
        else:
            QMessageBox.information(self, "Success", 
                                   f"Successfully saved analyses for {saved_count} well(s)!")
